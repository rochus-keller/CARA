...
namespace Spec
{
	...
	class StripScopeAgent2D;
	class Project;
	class Spectrum;
	using Root::Action;

	class StripScope2D : public Lexi::MainWindow
	{
	public:
		...
		StripScope2D(Root::Agent*,Project*,Spectrum*);
	protected:
		virtual ~StripScope2D();
	...
	private:
		...
		FRIEND_ACTION_HANDLER( StripScope2D );
		void buildViews();
		void buildMenus();
		void updateCaption();
		Root::Ref<Lexi::FocusManager> d_focus;
		Lexi::GlyphWidget* d_widget;
		Root::Ptr<StripScopeAgent2D> d_agent;
